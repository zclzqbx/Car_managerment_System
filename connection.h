...
static bool createConnection()
{
    ...
    db.setUserName("root");
    db.setPassword("riwfnyss");
    if(!db.open()){
        //提示出错
        //qDebug()<<"false is false"<<endl;
        return false;
    }
    /*else
        QMessageBox::information(0,QObject::tr("waring"),QObject::tr("data is open"));*/

    QSqlQuery query;

    //创建厂家表
    query.exec("create table factory(id varchar(20) primary key,name varchar(20))");
    ...
    //创建品牌表
    query.exec("create table brand(id varchar(20) primary key,name varchar(20),factory varchar(20),price int,sum int,sell int,last int)");
...
{
    QFile file("data.xml");
    if(file.exists())return true;//如果存在则直接返回
